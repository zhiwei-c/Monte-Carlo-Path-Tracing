...
public:
    QUALIFIER_DEVICE Integrator(float *pixel_buffer, Texture **texture_buffer, Bsdf **bsdf_buffer,
                                Primitive *primitive_buffer, Instance *instance_buffer, Accel *accel,
                                uint64_t num_emitter, Emitter **emitter_buffer,
                                uint64_t num_area_light, uint64_t *area_light_id_buffer,
                                EnvMap *env_map, Sun* sun);

    QUALIFIER_DEVICE Vec3 TraceRay(const Vec3 &eye, const Vec3 &look_dir, uint64_t *seed) const;

private:
    QUALIFIER_DEVICE Vec3 EvaluateDirectLight(const Intersection &its, const Vec3 &wo,
                                              uint64_t *seed) const;
    ...
    const uint64_t *area_light_id_buffer_;
    const Accel *accel_;
};