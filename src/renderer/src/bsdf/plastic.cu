...
QUALIFIER_D_H void rt::Bsdf::EvaluatePlastic(SamplingRecord *rec) const
{
    ...
    // 计算塑料清漆层和基底层反射的权重
    const Texture
        &diffuse = data_.texture_buffer[data_.plastic.id_diffuse_reflectance],
        &specular = data_.texture_buffer[data_.plastic.id_specular_reflectance];
    const Vec3 kd = diffuse.GetColor(rec->texcoord),
               ks = specular.GetColor(rec->texcoord);
    float weight_spec =
        (kd.x + kd.y + kd.z) / ((kd.x + kd.y + kd.z) + (ks.x + ks.y + ks.z));
    ...
    const Texture &rougness = data_.texture_buffer[data_.plastic.id_roughness];
                                           ...
                                           SamplingRecord *rec) const
{
    ...
    if (xi.x < pdf_spec)
    { // 抽样塑料清漆层
        SampleGgx(xi.y, xi.z, alpha, &h_local, &D);
    ...
    { // 抽样塑料基底层
        ...
        SampleHemisCos(xi.y, xi.z, &wi_local, &pdf_diff_local);
