...
QUALIFIER_DEVICE void RoughDiffuse::Evaluate(const float *pixel_buffer, Texture **texture_buffer,
...
QUALIFIER_DEVICE void RoughDiffuse::Sample(const float *pixel_buffer, Texture **texture_buffer,
                                           uint64_t *seed, SamplingRecord *rec) const
{
    // 余弦加权重要抽样入射光线的方向
