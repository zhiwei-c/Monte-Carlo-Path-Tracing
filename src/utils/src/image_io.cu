...
void image_io::Write(const int width, const int height, const float *frame_buffer,
                     const std::string &filename)
{
    ...
    for (int j = 0; j < height; ++j)
    {
        for (int i = 0; i < width; ++i)
        {
            ...
            color[offset + 1] = static_cast<unsigned char>(
                std::min(255, static_cast<int>(255 * frame_buffer[offset + 1])));
            color[offset + 2] = static_cast<unsigned char>(
                std::min(255, static_cast<int>(255 * frame_buffer[offset + 2])));
        }
    }
...
void image_io::Read(const std::string &filename, const float gamma, const int *width_max,
                    int *width, int *height, int *channel, std::vector<float> *data)
{
    const std::set<std::string> support_format = {
        "EXR", "exr", "jpg", "jpeg", "JPG", "JPEG", "png", "PNG", "tga", "TGA", "bmp", "BMP",
        "psd", "PSD", "gif", "GIF",  "hdr", "HDR",  "pic", "PIC", "pgm", "PGM", "ppm", "PPM"};
    ...
    if (width_max != nullptr && *width > *width_max)
    {
        ...
        Resize(raw_data, *width, *height, 0, target_data, *width_max, height_target, 0, *channel);
...
{
    stbir_resize_float_linear(input_pixels, input_w, input_h, input_stride_in_bytes, output_pixels,
