...
constexpr float RandomFloat(uint32_t *seed)
{
    *seed = *seed * 1664525u + 1013904223u;
    return static_cast<float>(*seed & 0x00ffffff) / static_cast<float>(0x01000000u);
}
...
template <typename T>
QUALIFIER_D_H T Lerp(const T *v, const float alpha, const float beta, const float gamma)
{
...
QUALIFIER_D_H bool SolveQuadratic(const float a, const float b, const float c, float *x0,
                                  float *x1);
