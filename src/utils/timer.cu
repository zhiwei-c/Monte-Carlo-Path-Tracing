...
void Timer::PrintTimePassed(const std::string &work_name)
{
    ...
    double diff = std::chrono::duration<double>(time_current_ - time_begin_).count();
    unsigned int hours = static_cast<unsigned int>(diff / 3600),
                 mins = static_cast<unsigned int>(diff / 60) - hours * 60,
                 secs = static_cast<unsigned int>(diff) - hours * 3600 - mins * 60,
                 ms = static_cast<unsigned int>((diff - static_cast<unsigned int>(diff)) * 1000);
    fprintf(stderr, "[info] \"%s\" finished. \n\tIt takes %u hr %u min %u sec %u ms.\n",
...
void Timer::PrintProgress(double progress)
{
    ...
    fprintf(stderr, "\r[info] %3.5f %%  [%d:%02d:%02d / %d:%02d:%02d] \t\r",
            progress * 100, hours, mins, secs,
            hours_predict, mins_predict, secs_predict);
}
