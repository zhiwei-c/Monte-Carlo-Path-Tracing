...
struct Mat4
{
    ...
    QUALIFIER_D_H Mat4(const Vec4 &row0, const Vec4 &row1, const Vec4 &row2, const Vec4 &row3);
    QUALIFIER_D_H Mat4(const float x0, const float y0, const float z0, const float w0,
    ...
    QUALIFIER_D_H Mat4 operator-() const { return {-rows[0], -rows[1], -rows[2], -rows[3]}; }
