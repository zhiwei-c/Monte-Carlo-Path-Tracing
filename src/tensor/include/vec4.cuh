...
struct Vec4
{
    ...
    QUALIFIER_D_H Vec4(const Vec3 &vec, float w) : x(vec.x), y(vec.y), z(vec.z), w(w) {}
    QUALIFIER_D_H Vec4(const float x, const float y, const float z, const float w)
        : x(x), y(y), z(z), w(w)
    {
    }
    ...
    QUALIFIER_D_H void operator=(const Vec4 &vec) { x = vec.x, y = vec.y, z = vec.z, w = vec.w; }
