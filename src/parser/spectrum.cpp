#include "csrt/parser/spectrum.hpp"
...
constexpr int kXyzSamples = 60;
constexpr int kXyzSampleWavelengthStart = 400;
constexpr int kXyzSampleWavelengthEnd = 700;
constexpr int kCieSamples = 471;
constexpr float kCieYIntegral = 106.856895;

constexpr float kCieWavelengths[kCieSamples] = {
...
constexpr float kCieX[kCieSamples] = {
...
constexpr float kCieY[kCieSamples] = {
...
constexpr float kCieZ[kCieSamples] = {
...
{
    const std::vector<float> cie_wavelengths = {kCieWavelengths,
                                                kCieWavelengths + kCieSamples},
                             cie_x = {kCieX, kCieX + kCieSamples};
    auto X = std::vector<float>(kXyzSamples);
    for (int i = 0; i < kXyzSamples; ++i)
    {
        float wavelength_0 =
            Lerp(kXyzSampleWavelengthStart, kXyzSampleWavelengthEnd,
                 static_cast<float>(i) / kXyzSamples);
        float wavelength_1 =
            Lerp(kXyzSampleWavelengthStart, kXyzSampleWavelengthEnd,
                 static_cast<float>(i + 1) / float(kXyzSamples));
        X[i] = AverageSpectrumSamples(cie_wavelengths, cie_x, wavelength_0,
                                      wavelength_1);
    }
    return X;
}
...
std::vector<float> InitCieY()
{
    ...
    for (int i = 0; i < kXyzSamples; ++i)
    {
        ...
        float wavelength_1 =
            Lerp(kXyzSampleWavelengthStart, kXyzSampleWavelengthEnd,
                 static_cast<float>(i + 1) / float(kXyzSamples));
        Y[i] = AverageSpectrumSamples(cie_wavelengths, cie_y, wavelength_0,
                                      wavelength_1);
    }
    return Y;
}
...
std::vector<float> InitCieZ()
{
    ...
    for (int i = 0; i < kXyzSamples; ++i)
    {
        ...
        float wavelength_1 =
            Lerp(kXyzSampleWavelengthStart, kXyzSampleWavelengthEnd,
                 static_cast<float>(i + 1) / float(kXyzSamples));
        Z[i] = AverageSpectrumSamples(cie_wavelengths, cie_z, wavelength_0,
                                      wavelength_1);
    }
    return Z;
}
                            ...
                            const float lambda)
{
    ...
    auto result =
        std::equal_range(wavelengths.begin(), wavelengths.end(), lambda);

    const size_t idx1 = result.first - wavelengths.begin(),
                 idx2 = result.second - wavelengths.begin();
    if (idx1 == idx2)
    {
        const float a = wavelengths[idx1 - 1], b = wavelengths[idx1],
                             ...
                             const float wavelength_end)
{
    ...
    if (wavelength_end > wavelengths[samples_num - 1])
        sum += amplitudes[samples_num - 1] *
               (wavelength_end - wavelengths[samples_num - 1]);

    auto interp = [wavelengths, amplitudes](float w, int i)
    {
        const float t =
            (w - wavelengths[i]) / (wavelengths[i + 1] - wavelengths[i]);
        return Lerp(amplitudes[i], amplitudes[i + 1], t);
    };
    ...
    for (; i + 1 < samples_num && wavelength_end >= wavelengths[i]; ++i)
    {
        ...
        sum += 0.5f *
               (interp(segwavelength_start, i) + interp(segwavelength_end, i)) *
...
Vec3 SpectrumToRgb(const std::vector<float> &wavelengths,
                   const std::vector<float> &amplitudes)
{
    static const std::vector<float> X = InitCieX(), Y = InitCieY(),
                                    Z = InitCieZ();
    ...
    for (int i = 0; i < kXyzSamples; ++i)
    {
        ...
        float wavelength_1 =
            Lerp(kXyzSampleWavelengthStart, kXyzSampleWavelengthEnd,
                 static_cast<float>(i + 1) / float(kXyzSamples));
        float amplitude = AverageSpectrumSamples(wavelengths, amplitudes,
    ...
    return {fmaxf(0.0f,
            ...
            fmaxf(0.0f, -0.969256f * xyz[0] + 1.875991f * xyz[1] +
                            0.041556f * xyz[2]),
            fmaxf(0.0f, 0.055648f * xyz[0] - 0.204043f * xyz[1] +
                            1.057311f * xyz[2])};
}
