...
constexpr float kKoWavelengths[64] = {
    300, 305, 310, 315, 320, 325, 330, 335, 340, 345, 350, 355, 445,
    450, 455, 460, 465, 470, 475, 480, 485, 490, 495, 500, 505, 510,
    515, 520, 525, 530, 535, 540, 545, 550, 555, 560, 565, 570, 575,
    580, 585, 590, 595, 600, 605, 610, 620, 630, 640, 650, 660, 670,
...
constexpr float kKoAmplitudes[64] = {
    ...
    .0,   .003, .003, .004, .006, .008, .009, .012, .014, .017, .021,
    .025, .03,  .035, .04,  .045, .048, .057, .063, .07,  .075, .08,
    .085, .095, .103, .110, .12,  .122, .12,  .118, .115, .12,  .125,
    .130, .12,  .105, .09,  .079, .067, .057, .048, .036, .028, .023,
    .018, .014, .011, .010, .009, .007, .004, .0,   .0};

// k_g Spectrum table from pg 130, MI.
constexpr float kKgWavelengths[4] = {759, 760, 770, 771};

constexpr float kKgAmplitudes[4] = {0, 3.0, 0.210, 0};

// k_wa Spectrum table from pg 130, MI.
constexpr float kKwaWavelengths[13] = {689, 690, 700, 710, 720, 730, 740,
                                       750, 760, 770, 780, 790, 800};

constexpr float kKwaAmplitudes[13] = {
    0,        0.160e-1, 0.240e-1, 0.125e-1, 0.100e+1, 0.870,   0.610e-1,
...
constexpr float kSolarAmplitudes[38] = {
    16559.0, 16233.7, 21127.5, 25888.2, 25829.1, 24232.3, 26760.5, 29658.3,
    30545.4, 30057.5, 30663.7, 28830.4, 28712.1, 27825.0, 27100.6, 27233.6,
    26361.3, 25503.8, 25060.2, 25311.6, 25355.9, 25134.2, 24631.5, 24173.2,
    23685.3, 23212.1, 22827.7, 22339.8, 21970.2, 21526.7, 21097.9, 20728.3,
    20240.4, 19870.8, 19427.2, 19072.4, 18628.9, 18259.2};

Vec3 GetSunRadiance(float theta, float turbidity)
{
    static std::vector<float>
        k_o_wavelengths = {kKoWavelengths,
                           kKoWavelengths + sizeof(kKoWavelengths) /
                                                sizeof(kKoWavelengths[0])},
        k_o_amplitudes = {kKoAmplitudes,
                          kKoAmplitudes +
                              sizeof(kKoAmplitudes) / sizeof(kKoAmplitudes[0])},
        k_g_wavelengths = {kKgWavelengths,
                           kKgWavelengths + sizeof(kKgWavelengths) /
                                                sizeof(kKgWavelengths[0])},
        K_g_values = {kKgAmplitudes,
                      kKgAmplitudes +
                          sizeof(kKgAmplitudes) / sizeof(kKgAmplitudes[0])},
        wa_wavelengths = {kKwaWavelengths,
                          kKwaWavelengths + sizeof(kKwaWavelengths) /
                                                sizeof(kKwaWavelengths[0])},
        wa_amplitudes = {kKwaAmplitudes,
                         kKwaAmplitudes + sizeof(kKwaAmplitudes) /
                                              sizeof(kKwaAmplitudes[0])},
        solar_wavelengths = {kSolarWavelengths,
                             kSolarWavelengths +
                                 sizeof(kSolarWavelengths) /
    ...
    float m = 1.0f / (std::cos(theta) +
                      0.15f * std::pow(93.885 - theta / kPi * 180.0, -1.253f));
    ...
    for (i = 0, lambda = 350; i < 91; ++i, lambda += 5)
    {
        ...
        float tau_rR =
            std::exp(-m * 0.008735f * std::pow(lambda / 1000.0f, -4.08f));
        ...
        const float k_o_amplitude =
            EvalSpectrumAmplitude(k_o_wavelengths, k_o_amplitudes, lambda);
        float tau_o = std::exp(-m * k_o_amplitude * l_ozone);

        // Attenuation due to mixed gases absorption
        // Results agree with the graph (pg 131, MI)
        const float k_g_amplitude =
            EvalSpectrumAmplitude(k_g_wavelengths, K_g_values, lambda);
        ...
        const float w = 2.0;
        const float wa_amplitude =
            EvalSpectrumAmplitude(wa_wavelengths, wa_amplitudes, lambda);
        float tau_va =
            std::exp(-0.2385 * wa_amplitude * w * m /
                     std::pow(1 + 20.07 * wa_amplitude * w * m, 0.45));

        wavelengths[i] = lambda;
        data[i] =
            EvalSpectrumAmplitude(solar_wavelengths, solar_amplitudes, lambda) *
...
Vec3 GetSunDirection(const LocationDate &location_date)
{
       ...
       and JD 2451545.0, which is noon 1 January 2000 Universal Time */
    {
        ...
        dec_hours =
            location_date.hour - location_date.timezone +
    ...
    // Calculate local coordinates (azimuth and zenith angle) in degrees
    {
        float greenwich_Mean_sidereal_time =
            6.6974243242f + 0.0657098283f * elapsed_Julian_days + dec_hours;

        float local_mean_sidereal_time = ToRadians(
            greenwich_Mean_sidereal_time * 15.0f + location_date.longitude);
    ...
    return {sinf(azimuth) * sinf(zenith), cosf(zenith),
                                    ...
                                    const int height, Vec3 *radiance)
{
    ...
    const Vec3 sun_radiance =
        ::GetSunRadiance(zenith, turbidity) * radiance_scale;
    *radiance = sun_radiance * solid_angle;

    const float cos_theta = cosf(theta * radius_scale),
                covered_portion =
                    0.5f *
                    (1.0f - cos_theta); // sphere ratio covered by the sun
    int sample_num = static_cast<int>(width * height * 500 * covered_portion);
    if (sample_num < 100)
        sample_num = 100;

    auto data = std::vector<float>(width * height * 3);
    ...
    for (int i = 0; i < sample_num; ++i)
    {
        Vec3 dir = SampleConeUniform(cos_theta, (i + 1.0f) / sample_num,
                                     GetVanDerCorputSequence<2>(i + 1));
        dir = LocalToWorld(dir, sun_direction);
        ...
        const int x = std::min(
                      std::max(0, static_cast<int>(local_azimuth * factor.u)),
                      width - 1),
                  y = std::min(
                      std::max(0, static_cast<int>(local_elevation * factor.v)),
                      height - 1);
                                    ...
                                    const int height)
{
    ...
    for (unsigned int i = 0; i < 3; i++)
    {
        skymodel_state[i] = arhosek_rgb_skymodelstate_alloc_init(
            turbidity, albedo[i], 0.5f * kPi - zenith);
    }
    ...
    for (int y = 0; y < height; ++y)
    {
        ...
        if (std::cos(theta) <= 0.0)
        {
            if (extend)
            {
                ...
                float temp1 = 2.0 - 2.0 * theta_raw * k1DivPi;
        ...
        for (int x = 0; x < width; ++x)
        {
            ...
            for (int i = 0; i < 3; ++i)
            {
                color[i] = static_cast<float>(arhosek_tristim_skymodel_radiance(
                               skymodel_state[i], theta, gamma, i)) /
                           106.856980f;
