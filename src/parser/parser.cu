#include "csrt/parser/parser.cuh"
...
#include "csrt/parser/ior_lut.cuh"
#include "csrt/parser/model_loader.cuh"
...
void element_parser::ReadCamera(const pugi::xml_node &sensor_node)
{
    ...
    if (type != "perspective")
    {
        throw std::exception("[error] only support 'perspective' sensor.\n");
    }
    //
    // 读取绘制图像的分辨率
    //
...
Config LoadConfig(const std::string &filename)
{
    ...
    {
        throw std::exception("[error] read config file failed.\n");
    }
