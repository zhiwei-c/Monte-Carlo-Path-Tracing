...
inline Medium *LookupHomogeneousMedium(const std::string &name)
{
    for (int i = 0; i < kMediumNum1; ++i)
    {
        if (name == std::string(kMediumData1[i].name))
            return new HomogeneousMedium(kMediumData1[i].sigma_s, kMediumData1[i].sigma_a,
                                         std::make_unique<IsotropicPhase>());
    }
    for (int i = 0; i < kMediumNum2; ++i)
    {
        if (name == std::string(kMediumData2[i].name))
            return new HomogeneousMedium(kMediumData1[i].sigma_s, kMediumData1[i].sigma_a,
                                         std::make_unique<HenyeyGreensteinPhase>(kMediumData1[i].g));
    }
    return nullptr;
}
