...
constexpr int kAlbedoResolution = 1024;

inline vec3 HammersleyVec2(uint32_t i, uint32_t N)
{
...
void CreateCosinAlbedoTexture(MicrofacetDistribType type,
                              Float alpha_u,
                              Float alpha_v,
                              std::vector<float> &albedo,
                              float &albedo_avg)
{
    constexpr Float resolution_step = 1.0 / kAlbedoResolution;
    ...
    albedo = std::vector<float>(kAlbedoResolution, 0);
    for (int j = 0; j < kAlbedoResolution; j++)
    {
        ...
        for (int i = 0; i < sample_count; i++)
        {
            ...
            SampleNormDistrib(type, alpha_u, alpha_v,
                              ...
                              facet_normal, pdf);
            auto cos_m_o = std::max(myvec::dot(wo, facet_normal), static_cast<Float>(0));
            auto cos_m_n = std::max(myvec::dot(macro_normal, facet_normal), static_cast<Float>(0));
        ...
        albedo[j] = std::min(albedo[j] * sample_count_inv, 1.0);
    }
    ...
    for (int j = 0; j < kAlbedoResolution; j++)
    {
        ...
        for (int i = 0; i < sample_count; i++)
        {
            ...
            auto cos_n_i = std::max(myvec::dot(-wi, macro_normal), static_cast<Float>(0));
