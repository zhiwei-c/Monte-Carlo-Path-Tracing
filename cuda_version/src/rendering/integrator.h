...
#include "../modeling/accelerator.h"
                                  ...
                                  curandState *local_rand_state) const
{
    ...
    while (depth < max_depth_ && (depth <= rr_depth_ || curand_uniform(local_rand_state) < pdf_rr_))
    {
        ...
        if (!b_rec.valid)
            break;
        auto cos_theta = abs(myvec::dot(b_rec.wi, its.normal()));
                                              ...
                                              const vec3 &wi) const
{
    ...
    if (cos_theta_prime < 0)
        return 0;
    auto pdf_area = its_pre.pdf_area() / emitter_num_;
    auto distance_sqr_ = its_pre.distance() * its_pre.distance();
                                              ...
                                              vec3 &L) const
{
    ...
    auto tmp = curand_uniform(local_rand_state) * emitter_num_;
    auto index = static_cast<uint>(tmp);
    if (index == emitter_num_)
        index--;
    index = emitter_idx_list_[index];
    auto emitter = shapebvh_list_[index];
    auto its_pre = Intersection();
    emitter.SampleP(its_pre, RandomVec3(local_rand_state));

    auto pdf_area = its_pre.pdf_area() / emitter_num_;

    auto d_vec = its.pos() - its_pre.pos();
