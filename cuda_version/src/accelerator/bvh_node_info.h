                              ...
                              const std::vector<BvhNodeInfo> &shape_info_list, std::vector<BvhNodeInfo> &bvhnode_info_list)
{
    ...
    if (begin + 1 == end)
    {
        bvhnode_info_list[bvhnode_idx] = BvhNodeInfo(true, bvhnode_idx, kUintMax, kUintMax, shape_info_list[begin].aabb,
                                                     shape_idx_list[begin], shape_info_list[begin].area);
        return;
    }
    ...
    BuildSceneBvhInfo(left_bvh_node_idx, begin, mid, shape_idx_list, shape_info_list, bvhnode_info_list);
    BuildSceneBvhInfo(right_bvh_node_idx, mid, end, shape_idx_list, shape_info_list, bvhnode_info_list);

    bvhnode_info_list[bvhnode_idx] = BvhNodeInfo(false, bvhnode_idx, left_bvh_node_idx, right_bvh_node_idx,
                              ...
                              std::vector<BvhNodeInfo> &bvhnode_info_list)
{
    ...
    if (begin + 1 == end)
    {
        bvhnode_info_list[bvhnode_idx] = BvhNodeInfo(true, bvhnode_idx, kUintMax, kUintMax,
    ...
    BuildShapeBvhInfo(left_bvhnode_idx, begin, mid, scene_mesh_idx_list, scene_mesh_aabb_list, scene_mesh_area_list,
                      bvhnode_info_list);
    BuildShapeBvhInfo(right_bvhmode_idx, mid, end, scene_mesh_idx_list, scene_mesh_aabb_list, scene_mesh_area_list,
                      bvhnode_info_list);

    bvhnode_info_list[bvhnode_idx] = BvhNodeInfo(false, bvhnode_idx, left_bvhnode_idx, right_bvhmode_idx,
                                                 aabb_now, kUintMax, area_now);
}