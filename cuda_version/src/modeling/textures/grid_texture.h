...
{
    auto value = ColorCheckerboard(texcoord).length(),
         value_u = ColorCheckerboard({texcoord.x + 1e-4, texcoord.y}).length(),
         value_v = ColorCheckerboard({texcoord.x, texcoord.y + 1e-4}).length();
