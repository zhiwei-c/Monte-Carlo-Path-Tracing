...
__global__ inline void CreateBsdfs(uint bsdf_num, BsdfInfo *bsdf_info_list, Texture *texture_list, Bsdf **bsdf_list)
{
...
__global__ inline void FreeBsdfs(uint bsdf_num, Bsdf **bsdf_list)
{
    if (threadIdx.x != 0 && blockIdx.x != 0)
        return;
