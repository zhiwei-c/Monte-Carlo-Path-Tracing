...
void VertexBufferThread(aiMesh *mesh, uint begin, uint end, gmat4 *to_world_pos, gmat4 *to_world_norm, bool face_normals,
                        uint old_v_num, std::vector<Vertex> &vertex_list);

void ProcessMesh(const aiScene *scene, aiMesh *mesh, gmat4 *to_world_pos, gmat4 *to_world_norm, bool face_normals,
                 std::vector<Vertex> &vertex_list, std::vector<uvec3> &idx_list);

void ProcessNode(const aiScene *scene, aiNode *node, gmat4 *to_world_pos, gmat4 *to_world_norm, bool face_normals,
                 std::vector<Vertex> &vertex_list, std::vector<uvec3> &idx_list);
...
void IndexBufferThread(aiMesh *mesh, uint begin, uint end, uint old_v_num, uint old_i_num,
                 ...
                 std::vector<Vertex> &vertex_list, std::vector<uvec3> &idx_list)
{
    ...
    for (uint i = 0; i < blcok_num; i++)
    {
        ...
        wokers.push_back(std::thread(VertexBufferThread, mesh, begin, end, to_world_pos, to_world_norm, face_normals,
    ...
    for (uint i = 0; i < blcok_num; i++)
    {
        ...
        wokers.push_back(std::thread(IndexBufferThread, mesh, begin, end, old_v_num, old_i_num, std::ref(idx_list)));
    }
