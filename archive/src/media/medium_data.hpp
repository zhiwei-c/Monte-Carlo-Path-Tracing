...
constexpr MediumInfo kMediumData1[] = {
...
constexpr MediumInfo kMediumData2[] = {
...
constexpr int kMediumNum1 = sizeof(kMediumData1) / sizeof(MediumInfo);
constexpr int kMediumNum2 = sizeof(kMediumData2) / sizeof(MediumInfo);

inline Medium *LookupHomogeneousMedium(const std::string &id, double scale, const std::string &name)
{
    for (int i = 0; i < kMediumNum1; ++i)
    {
        if (name == std::string(kMediumData1[i].name))
        {
            return new HomogeneousMedium(id, kMediumData1[i].sigma_a * scale, kMediumData1[i].sigma_s * scale,
                                         new IsotropicPhaseFunction());
        }
    }
    for (int i = 0; i < kMediumNum2; ++i)
    {
        if (name == std::string(kMediumData2[i].name))
        {
            return new HomogeneousMedium(id, kMediumData1[i].sigma_a * scale, kMediumData1[i].sigma_s * scale,
                                         new HenyeyGreensteinPhaseFunction(kMediumData1[i].g));
        }
    }
    return nullptr;
}
